...
connection_t.prototype.arduino_setup_complete=function()
{
	...
	this.network_getnext(robot_to_starpath(this.robot)+"pilot/power",function(power)
	{
		...
		if(power.reset&&power.reset.counter&&_this.reset_counter!=power.reset.counter) // check for location reset
		{
			_this.sensors.location.x=power.reset.location.x;
			_this.sensors.location.y=power.reset.location.y;
			_this.sensors.location.angle=power.reset.location.angle;
			_this.reset_counter=power.reset.counter;
